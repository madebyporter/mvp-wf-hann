      ...
      <aside ref="chatWindowEl" class="chat-window h-full min-h-0 overflow-y-auto p-4 space-y-4">
        ...
        <div v-for="(msg, i) in chatMessages" :key="i" class="flex" :class="msg.role === 'human' ? 'justify-start' : 'justify-end'">
          <div class="w-[80%] rounded-xl px-4 py-3 text-sm border" :class="msg.role === 'human' ? 'bg-white border-slate-300 text-slate-800' : 'bg-slate-100 border-slate-200 text-slate-700'">
            <p class="text-[11px] uppercase tracking-wide mb-1" :class="msg.role === 'human' ? 'text-slate-500' : 'text-blue-700'">{{ msg.role === 'human' ? 'Dispatcher' : 'AI Ops' }}</p>
            {{ msg.text }}
          </div>
        </div>
...
function botReply(input: string) {
  ...
  if (/(what can i set|help|commands|how do i use)/i.test(text) && !targetId) {
    return ['Absolutely — quick guide:', `Commercial status options: ${commercialStatusOptions.join(', ')}.`, `Emergency status options: ${emergencyStatusOptions.join(', ')}.`, 'You can ask things like: "what is being worked on for Lakewood School District?", "set PRJ-4101 to complete", or "where is EM-2042?"'].join(' ')
  }
  if (/\b(quote|quotes|conversion|won|renewals|scheduled visits|at-risk|capacity|crews assigned|pending permits)\b/.test(text)) {
    ...
    if (/how many.*quotes|quotes sent|quote count/.test(text)) return `Quotes sent so far: ${quoteConversion.quotesSent}. Won: ${quoteConversion.won}. Conversion: ${quoteConversion.conversion}%.`
    if (/how many.*won|jobs won|wins\b/.test(text)) return `Won so far: ${quoteConversion.won} out of ${quoteConversion.quotesSent} quotes (${quoteConversion.conversion}% conversion).`
  ...
  const opportunityByName = opportunities.value.find((opp) => { const company = opp.company.toLowerCase(); const first = company.split(' ')[0] ?? ''; return text.includes(company) || text.includes(first) })
  ...
  if (/recent opportunities|recent leads|show crm|open crm/.test(text)) {
    router.push('/crm')
    return `Opened CRM. Top recent opportunities are ${recentOpportunities.value.map((o) => o.id).join(', ')}.`
  }
  const projectByName = installJobs.value.find((job) => { const client = job.client.toLowerCase(); const location = job.location.toLowerCase(); return text.includes(client) || text.includes(location) })
  ...
  if (projectByName && isStatusIntent) {
    ...
    return [`Right now for ${projectByName.client}:`, `${projectByName.currentWork || projectByName.scope}`, `Current stage is ${projectByName.stage}.`, `Next up: ${projectByName.nextStep || 'confirm next milestone and crew timing.'}`, `If you want, I can change the stage label for ${projectByName.id} right now.`].join(' ')
  }
  if (!targetId) return 'I can help — tell me a code (EM/PRJ/OPP) or a client/company name, and I can update status/stage immediately.'
  if (/^OPP-\d+$/i.test(targetId)) {
    ...
    if (isStatusIntent || /\?$/.test(text)) return `${opp.id} for ${opp.company} is ${opp.stage}. Value is $${opp.value.toLocaleString()}.`
  }
  ...
  if (/^EM-\d+$/i.test(targetId)) {
    ...
    if (isStatusIntent || /\?$/.test(text)) return `${job.ticket} is currently ${job.status}. ETA target is ${job.eta} in ${job.city}.`
  }
