...
export function updateDealStage(
  ...
  return {
    state: next,
    systemMessages: [`Updated ${args.dealId} to ${args.dealStage}.`]
  }
}
...
export function updateDeal(
  ...
  return {
    state: next,
    systemMessages: [`Updated deal ${args.dealId}.`]
  }
}
...
export function upsertClient(
      ...
      ? state.clients.map((c) =>
          c.id === id
            ? {
                ...c,
                name,
                phone: args.phone ?? c.phone,
                email: args.email ?? c.email,
  ...
  const msg = existing
    ? `Updated client: ${name} (${id}).`
    : `Created client: ${name} (${id}).`
  return {
    state: { ...state, clients },
    systemMessages: [msg]
  }
...
export function dispatchEmergencyJob(
  ...
  const idx = list.findIndex((j) => j.ticket === args.jobId)
  if (idx < 0) {
    return { state, systemMessages: [`Job ${args.jobId} not found.`] }
  }
  const job = list[idx]
  ...
  if (args.assignCrew != null && args.assignCrew.trim()) {
    const crewId =
      state.crews.find(
        (c) =>
          c.id === args.assignCrew!.trim() ||
          c.name.toLowerCase() === args.assignCrew!.trim().toLowerCase()
  ...
  if (args.status != null && args.status.trim()) {
    const status =
      EMERGENCY_STATUSES.find(
        (s) => s.toLowerCase() === args.status!.trim().toLowerCase()
      ) ?? args.status.trim()
    updated.status = status
    const etaStr =
      args.etaTargetMinutes != null
        ? `, ETA ${args.etaTargetMinutes} min`
        : updated.eta
        ? `, ETA ${updated.eta}`
        : ''
    messages.push(`Updated status: ${status}${etaStr}.`)
  ...
  if (args.notifyCustomer) {
    const { state: afterNotif, systemMessages: notifMsgs } = queueNotification(
      next,
      { jobId: args.jobId, type: 'customer_eta', payload: { note: args.note } }
    )
    next = afterNotif
...
export function queueNotification(
  ...
  return {
    ...
    systemMessages: ['Notifications queued.']
  }
}
...
export function validateDealStage(s: string): DealStage | null {
  const v = DEAL_STAGES.find((x) => x.toLowerCase() === s.trim().toLowerCase())
  return v ?? null
}

export function validateServicePlanTier(s: string): ServicePlanTier | null {
  const v = SERVICE_TIERS.find((x) => x.toLowerCase() === s.trim().toLowerCase())
  return v ?? null
}
